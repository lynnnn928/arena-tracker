...
async function getStats(tank?: string) {
  const tankName = tank || "RedStar";
  return apiGet(`/api/stats/${encodeURIComponent(tankName)}`);
}

async function getVersionSummary(tank?: string) {
  const tankName = tank || "RedStar";
  return apiGet(`/api/versions/${encodeURIComponent(tankName)}`);
}

async function getMapStats(tank?: string) {
  const tankName = tank || "RedStar";
  return apiGet(`/api/maps/${encodeURIComponent(tankName)}`);
}

async function compareVersions(cv1: number, cv2: number, tank?: string, map?: string) {
  const tankName = tank || "RedStar";
  let path = `/api/compare/${encodeURIComponent(tankName)}?cv1=${cv1}&cv2=${cv2}`;
  if (map) path += `&map=${encodeURIComponent(map)}`;
  return apiGet(path);
}

async function getRankHistory(tank?: string, limit?: number) {
  const tankName = tank || "RedStar";
  const lim = limit || 50;
  return apiGet(`/api/rank-history/${encodeURIComponent(tankName)}?limit=${lim}`);
}

async function getMatches(tank?: string, options?: { map?: string; cv?: number; won?: boolean; limit?: number }) {
  const tankName = tank || "RedStar";
  const lim = options?.limit || 50;
  let path = `/api/matches/${encodeURIComponent(tankName)}?limit=${lim}`;
  if (options?.map) path += `&map=${encodeURIComponent(options.map)}`;
  if (options?.cv) path += `&cv=${options.cv}`;
  if (options?.won !== undefined) path += `&won=${options.won}`;
  return apiGet(path);
}
...
async function getOpponents(tank?: string, limit?: number) {
  const tankName = tank || "RedStar";
  const lim = limit || 20;
  return apiGet(`/api/opponents/${encodeURIComponent(tankName)}?limit=${lim}`);
}

async function getVersionDetail(cv: number, tank?: string) {
  const tankName = tank || "RedStar";
  const versions = await apiGet(`/api/versions/${encodeURIComponent(tankName)}`) as any;
  const version = versions.versions?.find((v: any) => v.cv === cv);
  const maps = await apiGet(`/api/maps/${encodeURIComponent(tankName)}`) as any;
