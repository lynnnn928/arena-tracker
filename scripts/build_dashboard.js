...
<table><tr><th data-i18n="charts.versionMap">版本 / 地图</th>${tank.matrix.maps.map(m => `<th>${shortMapName(m)}</th>`).join('')}<th data-i18n="charts.total">合计</th></tr>
...
${tank.mapBreakdown.map(function(m) {
  ...
  if (data.mapCache && data.mapCache.length > 0) {
    h += `<div class="card"><h3 data-i18n="maps.info">地图信息（从回放推算）</h3><table><tr><th data-i18n="maps.mapName">地图</th><th data-i18n="maps.size">尺寸</th><th data-i18n="maps.cells">格数</th><th data-i18n="maps.strategy">策略分类</th></tr>`;
...
<option value="" data-i18n="rank.allTiers">所有段位</option>
...
<option value="" data-i18n="rank.allCv">所有版本</option>
${tank.versionSummary.map(v => `<option value="${v.cv}">v${v.cv}</option>`).join('')}
</select>
<button id="rankSortTimeAsc-${tank.name}" class="active" onclick="sortRankTimeline('${tank.name}','timeAsc')" data-i18n="rank.sortTimeAsc">时间 ↑</button>
<button id="rankSortTimeDesc-${tank.name}" onclick="sortRankTimeline('${tank.name}','timeDesc')" data-i18n="rank.sortTimeDesc">时间 ↓</button>
</div>
...
function renderRankChartWithSample(t, mode) {
  ...
  charts[t.name + '-rank-full'] = new Chart(ctx, { type: 'line', data: {
    ...
    datasets: [{ label: i18n('rank.rankScore') + ' (' + i18n('rank.' + mode).toLowerCase() + ')', data: data.map(function(r) { return r.rankScore; }),
...
function renderRankTimeline(tankName, mode) {
  ...
  var table = '<table><tr><th>' + i18n('rank.time') + '</th><th>' + i18n('rank.rankScore') + '</th><th>' + i18n('rank.tier') + '</th><th>' + i18n('rank.cv') + '</th></tr>';
  ...
  document.getElementById('rankPageInfo-' + tankName).textContent = i18n('rank.page') + ' ' + (page + 1) + ' / ' + pages + ' (' + total + ' ' + i18n('rank.points') + ')';
...
function renderMatchTable(tankName) {
  ...
  var table = '<table><tr><th>' + i18n('matches.time') + '</th><th>' + i18n('matches.map') + '</th><th>' + i18n('matches.cv') + '</th><th>' + i18n('matches.result') + '</th><th>' + i18n('matches.reason') + '</th><th>' + i18n('matches.shots') + '</th><th>' + i18n('matches.stars') + '</th><th>' + i18n('matches.source') + '</th><th>' + i18n('matches.rankDelta') + '</th><th>' + i18n('matches.opponent') + '</th></tr>';
  items.forEach(function(m) {
    var ts = m.ts ? m.ts.slice(5,19).replace('T', ' ') : '';
    table += '<tr><td style="font-size:11px;color:#6b5c4a">' + ts + '</td>' +
      '<td>' + shortMapName(m.mapId) + '</td>' +
      '<td>' + (m.cv || '-') + '</td>' +
      '<td class="' + (m.won ? 'win' : 'loss') + '">' + (m.won ? i18n('matches.winShort') : i18n('matches.lossShort')) + '</td>' +
      '<td>' + i18n('reasons.' + (m.reason || '-')) + '</td>' +
      '<td>' + (m.myShots || 0) + '</td>' +
      '<td>' + (m.meStars || 0) + '</td>' +
  ...
  document.getElementById('pageInfo-' + tankName).textContent = i18n('matches.page') + ' ' + (page + 1) + ' / ' + pages + ' (' + list.length + ' ' + i18n('matches.matchesCount') + ')';
